    ...
    for i in 0..m{
        for j in 0..n{
            if seq1[i] == seq2[j]
                ...
                {C.set(i+1,j+1,std::cmp::max(C.get(i+1,j), C.get(i,j+1)).unwrap());}
        }
    }
    return C
}
...
fn print_diff(lcs_table: &Grid, lines1: &Vec<String>, lines2: &Vec<String>, i: usize, j: usize) {
    if i > 0 && j > 0 && lines1[i-1] == lines2[j-1] {
        ...
        println!(" {}", lines1[i-1]);
    } else if j > 0 && (i == 0 || (j > 0 && i > 0 && lcs_table.get(i, j-1).unwrap() >= lcs_table.get(i-1, j).unwrap())) {
        print_diff(lcs_table, lines1, lines2, i, j-1);
        println!("> {}", lines2[j-1]);
    } else if i > 0 && (j == 0 || (j > 0 && i > 0 && lcs_table.get(i, j-1).unwrap() < lcs_table.get(i-1, j).unwrap())) {
...
fn main() {
    ...
    print_diff(&C, &lines1, &lines2,1,2);
}
